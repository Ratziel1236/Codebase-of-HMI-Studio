...
ll a,b,c,ra,rb,rc,cnt;
int main()
{
	a=rd();b=rd();c=rd();if((a|b|c)&1){prt(0);return 0;}
	while(1)
	{
		ra=(b+c)>>1,rb=(a+c)>>1,rc=(a+b)>>1;++cnt;
		if((ra|rb|rc)&1){prt(cnt);return 0;}
		if(umin(a,umin(b,c))==umin(ra,umin(rb,rc))&&
			umax(a,umax(b,c))==umax(ra,umax(rb,rc))){prt(-1);return 0;}
		a=ra,b=rb,c=rc;
	}
	return 0;
}
